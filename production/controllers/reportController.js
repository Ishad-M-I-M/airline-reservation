...
router.post("/total-revenue", (req, res) => {
    if (req.body.aircraft_id === "") {
        db.raw("SELECT aircraft.model, SUM(ticket.paid) AS Total_Revenue FROM ticket INNER JOIN flight USING(flight_id) INNER JOIN aircraft USING(aircraft_id) WHERE flight.takeoff_time < NOW() AND is_boarded = 1 GROUP BY aircraft.model;",).then((result) => {
            return res.json({success: true, data: result[0]});
        })
            .catch((err) => {
                console.error(err);
        ...
        db.raw("SELECT aircraft.model, SUM(ticket.paid) as Total_Revenue FROM ticket INNER JOIN flight USING(flight_id) INNER JOIN aircraft USING(aircraft_id) WHERE flight.takeoff_time < NOW() AND is_boarded = 1 AND aircraft.model = ?", [req.body.aircraft_id],).then((result) => {
            return res.json({success: true, data: result[0]});
        })
            .catch((err) => {
                console.error(err);
...
router.post("/passenger-count", (req, res) => {
    if (req.body.Destination === "") {
        if (req.body.Enddate !== "" && req.body.Startdate !== "") {
            ...
            db.raw("SELECT airport.code, airport.name, COUNT(ticket.is_boarded) AS Passenger_Count FROM flight INNER JOIN route USING(route_id) INNER JOIN airport ON route.destination = airport.airport_id INNER JOIN ticket USING(flight_id) WHERE NOT (flight.takeoff_time < ?) AND ticket.is_boarded = 1 GROUP BY airport.code;", [req.body.Startdate],).then((result) => {
                return res.json({success: true, data: result[0]});
            }).catch((err) => {
                console.error(err);
                return res.status(500).json({success: false});
            ...
            db.raw("SELECT airport.code, airport.name, COUNT(ticket.is_boarded) AS Passenger_Count FROM flight INNER JOIN route USING(route_id) INNER JOIN airport ON route.destination = airport.airport_id INNER JOIN ticket USING(flight_id) WHERE ticket.is_boarded = 1 GROUP BY airport.code;",).then((result) => {
                return res.json({success: true, data: result[0]});
            }).catch((err) => {
                console.error(err);
                return res.status(500).json({success: false});
        ...
        if (req.body.Enddate !== "" && req.body.Startdate !== "") {
            ...
            db.raw("SELECT airport.code, airport.name, COUNT(ticket.is_boarded) AS Passenger_Count FROM flight INNER JOIN route USING(route_id) INNER JOIN airport ON route.destination = airport.airport_id INNER JOIN ticket USING(flight_id) WHERE airport.code = ? AND ticket.is_boarded = 1;", [req.body.Destination]).then((result) => {
                return res.json({success: true, data: result[0]});
            }).catch((err) => {
                console.error(err);
                return res.status(500).json({success: false});
...
router.post("/bookings", (req, res) => {
    if (req.session.userID) {
        if (req.body.Enddate === "" && req.body.Startdate !== "") {
            db.raw("SELECT discount.type, COUNT(ticket.user_id) AS Booking_Count FROM discount LEFT JOIN user ON discount.type = user.discount_type LEFT JOIN ticket USING(user_id) WHERE NOT (ticket.date < ?) GROUP BY type;", [req.body.Startdate]).then((result) => {
                return res.json({success: true, data: result[0]});
            }).catch((err) => {
                console.error(err);
                return res.status(500).json({success: false});
            ...
            db.raw("SELECT discount.type, COUNT(ticket.user_id) AS Booking_Count FROM discount LEFT JOIN user ON discount.type = user.discount_type LEFT JOIN ticket USING(user_id) WHERE NOT (ticket.date > ?)  GROUP BY type;", [req.body.Enddate]).then((result) => {
                return res.json({success: true, data: result[0]});
            }).catch((err) => {
                console.error(err);
                return res.status(500).json({success: false});
            ...
            db.raw("SELECT discount.type, COUNT(ticket.user_id) AS Booking_Count FROM discount LEFT JOIN user ON discount.type = user.discount_type LEFT JOIN ticket USING(user_id) WHERE NOT (ticket.date > ? OR ticket.date < ?) GROUP BY type;", [req.body.Enddate, req.body.Startdate]).then((result) => {
                return res.json({success: true, data: result[0]});
            }).catch((err) => {
                console.error(err);
                return res.status(500).json({success: false});
            ...
            db.raw("SELECT discount.type, COUNT(ticket.user_id) AS Booking_Count FROM discount LEFT JOIN user ON discount.type = user.discount_type LEFT JOIN ticket USING(user_id) GROUP BY type;",).then((result) => {
                return res.json({success: true, data: result[0]});
            }).catch((err) => {
                console.error(err);
                return res.status(500).json({success: false});
